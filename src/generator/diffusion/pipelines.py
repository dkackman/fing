...
class Pipelines:
    ...
    def preload_pipelines(self, auth_token, pipeline_names = ["txt2img", "img2img", "imginpaint"], conserve_memory = True):
        ...
        for pipe_line_name in pipeline_names:
            ...
            self.serialize_pipeline(pipeline, pipe_line_name, "fp16" if conserve_memory else "full")                     
    ...
    def serialize_pipeline(self, pipeline, pipeline_name, revision):
        ...
        pickle.dump(pipeline, open(f"/tmp/{model_name_path_part}.{revision}.{pipeline_name}.pipeline", "wb"))

        # open aand lock the file for later use
        self.files[pipeline_name] = open(f"/tmp/{model_name_path_part}.{revision}.{pipeline_name}.pipeline", "rb")
