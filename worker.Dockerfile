...
RUN /miniconda/bin/conda install pytorch torchvision torchaudio pytorch-cuda=11.6 -c pytorch -c nvidia
RUN /miniconda/bin/conda install xformers transformers accelerate scipy ftfy diffusers[torch] \
concurrent-log-handler fastapi gunicorn uvicorn pydantic pip -c conda-forge -c conda-forge -c xformers/label/dev
